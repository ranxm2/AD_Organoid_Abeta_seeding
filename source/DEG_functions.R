...
plot_gsva_boxplot_mutil_7<-  function(gsva_matrix, condition_list_label, pathway_name,
                                       figure_folder, file_name, 
                                       fig.height = 6, fig.width = 8,save = FALSE,
                                       reference_group_1, compare_group_1,p_value_1,
                                       reference_group_2, compare_group_2,p_value_2,
                                       reference_group_3, compare_group_3,p_value_3,
                                       reference_group_4, compare_group_4,p_value_4,
                                       reference_group_5, compare_group_5,p_value_5,
                                       reference_group_6, compare_group_6,p_value_6,
                                       reference_group_7, compare_group_7,p_value_7){
  ...
  # define your seven comparisons and p‐values:
  comparisons_list <- list(
    c(reference_group_1, compare_group_1),
    c(reference_group_2, compare_group_2),
    c(reference_group_3, compare_group_3),
    c(reference_group_4, compare_group_4),
    c(reference_group_5, compare_group_5),
    c(reference_group_6, compare_group_6),
    c(reference_group_7, compare_group_7)
  )
  ...
  # stagger the y‐positions so the brackets don’t overlap
  # define your four bracket heights
  h1 <- score_max + 0.2 * score_scale  # for comparisons 1 & 2
  h2 <- score_max + 0.4 * score_scale  # for comparisons 3 & 4
  h3 <- score_max + 0.6 * score_scale  # for comparisons 5 & 6
  h4 <- score_max + 0.8 * score_scale  # for comparison 7
  
  # assign to each comparison
  y_positions <- c(
    h1, # comp 1
    h1, # comp 2
    h2, # comp 3
  ...
  # 1. create stars/”ns” from your p-values
  sig_symbols <- symnum(
    c(p_value_1, p_value_2, p_value_3,
      p_value_4, p_value_5, p_value_6, p_value_7),
    cutpoints = c(0, 0.001, 0.01, 0.05, 1),
    symbols   = c("***", "**", "*", "N.S.")
  )
  
  
  # now use in your plot:
  p<-ggplot(plot_df, aes(group, GSVA_score, fill = group)) +
    stat_summary(fun = mean, geom = "bar", color = "black", alpha = 0.9, width = 0.7) +
    # stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.2) +
    geom_signif(
      comparisons      = comparisons_list,
      annotations      = as.character(sig_symbols),
      map_signif_level = FALSE,        # we already did the mapping
